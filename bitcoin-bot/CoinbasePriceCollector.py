...
class priceCollector: 
	...
	def record_price(self):
		...
		if self.times_run % self.outfileLength == 0: 
			print 'closing in mod'
			self.out.close()
			self.out = open(self.outfile + str(self.times_run / self.outfileLength) + '.csv', 'w')
		...
		self.out.write('{0},{1},{2}\n'.format(int(round(time.time())), buyPrice, sellPrice))
		self.times_run += 1 
