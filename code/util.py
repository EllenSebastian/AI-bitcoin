...
def create_weighted_csp():
    ...
    csp.add_unary_potential('A', lambda x : x > 1) # 1 or 0
    csp.add_unary_potential('A', lambda x : x != 2) # 1 or 0
    csp.add_unary_potential('B', lambda y : 1.0 / y) # inverse of y - weighted
    csp.add_binary_potential('A', 'B', lambda x, y : x != y) # 1 or 0 
    return csp
