...
"""
test = RuleBasedActionPicker(nBTC = 3, boughtAt = 300, maxnBTC = 10, buySellStep = 1, actualPrices = priceData, timestep = 60, predictionMethod = 'perfectRandom')
profits = test.randomSimulate(timestep=3600,ntimes = 24 * 30, n=100) # 1.0158806245527092
--> 1.3709887205882032 profit (make a decision every 1 hour for a month)
"""


"""
test = RuleBasedActionPicker(nBTC = 3, boughtAt = 300, maxnBTC = 10, buySellStep = 1, actualPrices = priceData, timestep = 60, predictionMethod = 'perfectRandom')
profits = test.randomSimulate(timestep=3600 * 24,ntimes = 24 * 30, n=100) # 1.0158806245527092
pdb.set_trace()
--> 49.216658575718093 profit (make a decision every 1 day for a 2 years)
--> 

PROBLEM: range(1248110820,1310232420 + 86400,86400) , holds on to money and does not sell.
"""


#""" try every hour for a month but only after the crash stabilized
t_range = range(1402692000,1405284000,60)
test = RuleBasedActionPicker(nBTC = 10, boughtAt = priceData[t_range[0]], maxnBTC = 10, buySellStep = 1, actualPrices = priceData, timestep = 60, predictionMethod = 'perfect')
#profits = test.randomSimulate(timestep=3600,ntimes = 24  , n=100, min_ts = 1387174080) # 1.0158806245527092
#print test.simulate(range(1415363940,1415663940,60), priceData)
print test.simulate(t_range, priceData)
print priceData[1402692000], priceData[1405284000]
#pdb.set_trace()
#"""
# with limits after dec 16 2013: 
# above with timestep=3600, ntimes = 24 * 30 --> 1.0104 profit
# above with timestep=3600, ntimes = 24 --> 1.006 profit always
# above with timestep=60, ntimes = 24 * 60 --> 1.0013778448332979

# without limits after dec 16 2013:
#print profits
